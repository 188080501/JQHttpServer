...
int main(int argc, char *argv[])
{
    ...
    JQHttpServer::SslServerManage tcpServerManage( 2 ); // 设置最大处理线程数，默认2个
    ...
    tcpServerManage.setHttpAcceptedCallback( []( const QPointer< JQHttpServer::Session > &session )
    {
        ...
        session->replyText( QString( "url:%1\ndata:%2" ).arg( session->requestUrl(), QString( session->requestRawData() ) ) );
...
//        session->replyImage( QImage( "/Users/jason/Desktop/Test.png" ) );
    ...
    qDebug() << "listen:" << tcpServerManage.listen( QHostAddress::Any, 24684, ":/server.crt", ":/server.key" );
